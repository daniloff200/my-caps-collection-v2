...
const COUNTRY_TO_ALPHA2: Record<string, string> = {
  ...
  'Czech Republic': 'CZ', 'DDR': 'DE', 'Democratic Republic of the Congo': 'CD',
  ...
  'Solomon Islands': 'SB', 'South Africa': 'ZA', 'South Korea': 'KR',
  ...
  'United Kingdom': 'GB', 'Uruguay': 'UY', 'USA': 'US', 'USSR': 'RU',
  'Uzbekistan': 'UZ', 'Vanuatu': 'VU', 'Venezuela': 'VE', 'Vietnam': 'VN',
  'Virgin Islands, U.S.': 'VI', 'Yemen': 'YE', 'Yugoslavia': 'RS',
...
export class CountryFlagEmojiPipe implements PipeTransform {
  transform(country: string): string {
    if (SPECIAL_EMOJI[country]) return SPECIAL_EMOJI[country];
    const code = COUNTRY_TO_ALPHA2[country];
